...
.traj_B_server <- function(
  ...
  observeEvent(input$traj_B_si_suffix, {
    if (!identical(input$traj_B_si_suffix, traj_B_values$si_suffix_current)) {
      traj_B_change$si_suffix_changed = TRUE
    }
    else {
      traj_B_change$si_suffix_changed = FALSE
    }
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$confirm_button, {
    ...
    if (
      ...
      y_min <- input$traj_B_y_min
      y_max <- input$traj_B_y_max
      ...
      if (!is.na(y_min) && !is.na(y_max) && y_min > y_max) {
        ...
        y_max <- tmp_y
        updateSelectInput(
          session, inputId = "traj_B_y_min", selected = y_min
        )
        updateSelectInput(
          session, inputId = "traj_B_y_max", selected = y_max
        )
        showNotification(
      ...
      if (!is.na(x_min) && !is.na(x_max) && x_min > x_max) {
        ...
        x_max <- tmp_x
        updateSelectInput(
          session, inputId = "traj_B_x_min", selected = x_min
        )
        updateSelectInput(
          session, inputId = "traj_B_x_max", selected = x_max
        )
        showNotification(
      ...
      traj_B_values$x_min_current = x_min
      traj_B_values$x_max_current = x_max
      traj_B_values$y_min_current = y_min
      traj_B_values$y_max_current = y_max
      traj_B_values$si_suffix_current = input$traj_B_si_suffix
      ...
      filtered_traj_B(
        traj_df %>%
          filter(Scenario %in% input$traj_B_scenarios) %>%
          droplevels()
      )
      title_x_traj_B(input$traj_B_title_x)
      title_y_traj_B(input$traj_B_title_y)
      palette_traj_B(input$traj_B_color)
      x_lim_min_traj_B(x_min)
      x_lim_max_traj_B(x_max)
      y_lim_min_traj_B(y_min)
      y_lim_max_traj_B(y_max)
      si_suffix_traj_B(input$traj_B_si_suffix)
    }
  }, ignoreInit = TRUE)
