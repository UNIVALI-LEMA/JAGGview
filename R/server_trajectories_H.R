...
.traj_H_server <- function(
  ...
  title_x_traj_H <- reactiveVal("Year")

  title_y_traj_H <- reactiveVal("Harvest rate")

  palette_traj_H <- reactiveVal("#1B4F8A")

  x_lim_min_traj_H <- reactiveVal(
    min((traj_df %>% filter(indicator == "H"))$year, na.rm = TRUE)
  )

  x_lim_max_traj_H <- reactiveVal(
    max((traj_df %>% filter(indicator == "H"))$year, na.rm = TRUE)
  )

  y_lim_min_traj_H <- reactiveVal(
    .round_to_nearest(
      min((traj_df %>% filter(indicator == "H"))$lcl, na.rm = TRUE), 
      FALSE, 1.1
    )
  )

  y_lim_max_traj_H <- reactiveVal(
    .round_to_nearest(
      max((traj_df %>% filter(indicator == "H"))$ucl, na.rm = TRUE), 
      TRUE, 1.1
    )
  )
  ...
  traj_H_values <- reactiveValues(
    ...
    x_min_current = min(
      (traj_df %>% filter(indicator == "H"))$year, na.rm = TRUE
    ),
    x_max_current = max(
      (traj_df %>% filter(indicator == "H"))$year, na.rm = TRUE
    ),
  ...
  observeEvent(input$traj_H_x_min, {
    if (input$traj_H_x_min != traj_H_values$x_min_current) {
  ...
  observeEvent(input$traj_H_x_max, {
    if (input$traj_H_x_max != traj_H_values$x_max_current) {
  ...
  observeEvent(input$traj_H_y_min, {
    if (
      !isTRUE(
        all.equal(input$traj_H_y_min, traj_H_values$y_min_current)
      )
    ) {
  ...
  observeEvent(input$traj_H_y_max, {
    if (
      !isTRUE(
        all.equal(input$traj_H_y_max, traj_H_values$y_max_current)
      )
    ) {
  ...
  observeEvent(input$confirm_button, {
    if (
      ...
      x_min <- .validate_year(input$traj_H_x_min, "traj_H_x_min", session)
      x_max <- .validate_year(input$traj_H_x_max, "traj_H_x_max", session)
  ...
  output$trajectories_H <- renderPlotly({
    ...
    min_x <- x_lim_min_traj_H()
    max_x <- x_lim_max_traj_H()
    range <- max_x - min_x

    x_lim <- .expand_range(c(min_x, max_x))
    y_lim <- .expand_range(c(y_lim_min_traj_H(), y_lim_max_traj_H()))

    si_suffix <- si_suffix_traj_H()
      ...
      layout(
        annotations = list(
          list(
            ...
            xref = "paper",
            yref = "paper",
            text = .format_title(title_x_traj_H()),
            showarrow = FALSE,
            font = list(
              size = 20
            )
          ...
          list(
            ...
            xref = "paper",
            yref = "paper",
            text = .format_title(title_y_traj_H()),
            showarrow = FALSE,
            font = list(
              size = 20
            )
