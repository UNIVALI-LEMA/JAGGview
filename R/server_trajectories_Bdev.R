...
.traj_Bdev_server <- function(
  ...
  observeEvent(input$traj_Bdev_scenarios, {
    if (!setequal(input$traj_Bdev_scenarios, 
      ...
      traj_Bdev_change$scenarios_changed = FALSE
    }
  }, ignoreInit = TRUE)

  observeEvent(input$traj_Bdev_title_x, {
    if (!identical(input$traj_Bdev_title_x, traj_Bdev_values$title_x_current)) {
      traj_Bdev_change$title_x_changed = TRUE
    }
    else {
      traj_Bdev_change$title_x_changed = FALSE
    }
  }, ignoreInit = TRUE)

  observeEvent(input$traj_Bdev_title_y, {
    if (!identical(input$traj_Bdev_title_y, traj_Bdev_values$title_y_current)) {
      traj_Bdev_change$title_y_changed = TRUE
    }
    else {
      traj_Bdev_change$title_y_changed = FALSE
    }
  }, ignoreInit = TRUE)

  observeEvent(input$traj_Bdev_color, {
    if (!identical(input$traj_Bdev_color, traj_Bdev_values$color_current)) {
      traj_Bdev_change$color_changed = TRUE
    }
    else {
      traj_Bdev_change$color_changed = FALSE
    }
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$traj_Bdev_x_min, {
    if (!identical(input$traj_Bdev_x_min, traj_Bdev_values$x_min_current)) {
      ...
      traj_Bdev_change$x_min_changed = FALSE
    }
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$traj_Bdev_x_max, {
    if (!identical(input$traj_Bdev_x_max, traj_Bdev_values$x_max_current)) {
      ...
      traj_Bdev_change$x_max_changed = FALSE
    }
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$traj_Bdev_y_min, {
    if (!identical(input$traj_Bdev_y_min, traj_Bdev_values$y_min_current)) {
      ...
      traj_Bdev_change$y_min_changed = FALSE
    }
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$traj_Bdev_y_max, {
    if (!identical(input$traj_Bdev_y_max, traj_Bdev_values$y_max_current)) {
      ...
      traj_Bdev_change$y_max_changed = FALSE
    }
  }, ignoreInit = TRUE)

  observeEvent(input$traj_Bdev_si_suffix, {
    if (
      !identical(input$traj_Bdev_si_suffix, traj_Bdev_values$si_suffix_current)
    ) {
      traj_Bdev_change$si_suffix_changed = TRUE
    }
    else {
      traj_Bdev_change$si_suffix_changed = FALSE
    }
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$confirm_button, {
    ...
    if (
      ...
      y_min <- input$traj_Bdev_y_min
      y_max <- input$traj_Bdev_y_max
      ...
      if (!is.na(y_min) && !is.na(y_max) && y_min > y_max) {
        ...
        y_max <- tmp_y
        updateSelectInput(
          session, inputId = "traj_Bdev_y_min", selected = y_min
        )
        updateSelectInput(
          session, inputId = "traj_Bdev_y_max", selected = y_max
        )
        showNotification(
      ...
      if (!is.na(x_min) && !is.na(x_max) && x_min > x_max) {
        ...
        x_max <- tmp_x
        updateSelectInput(
          session, inputId = "traj_Bdev_x_min", selected = x_min
        )
        updateSelectInput(
          session, inputId = "traj_Bdev_x_max", selected = x_max
        )
        showNotification(
      ...
      traj_Bdev_values$color_current = input$traj_Bdev_color
      traj_Bdev_values$x_min_current = x_min
      traj_Bdev_values$x_max_current = x_max
      traj_Bdev_values$y_min_current = y_min
      traj_Bdev_values$y_max_current = y_max
      traj_Bdev_values$si_suffix_current = input$traj_Bdev_si_suffix
      ...
      title_y_traj_Bdev(input$traj_Bdev_title_y)
      palette_traj_Bdev(input$traj_Bdev_color)
      x_lim_min_traj_Bdev(x_min)
      x_lim_max_traj_Bdev(x_max)
      y_lim_min_traj_Bdev(y_min)
      y_lim_max_traj_Bdev(y_max)
      si_suffix_traj_Bdev(input$traj_Bdev_si_suffix)
    }
  }, ignoreInit = TRUE)
