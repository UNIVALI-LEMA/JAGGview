#' @keywords internal
.retrospective_analysis_FFmsy_server <- function(
  ...
  si_suffix_ra_FFmsy <- reactiveVal(use_si_suffix)
  ...
  observeEvent(input$confirm_button, {
    if (
      ...
      ra_FFmsy_values$title_y_current = input$ra_FFmsy_title_y
      ra_FFmsy_values$text_size = input$ra_FFmsy_text_size
      ra_FFmsy_values$x_min_current = x_min
      ra_FFmsy_values$x_max_current = x_max
      ra_FFmsy_values$y_min_current = y_min
      ra_FFmsy_values$y_max_current = y_max
      ra_FFmsy_values$position_current = input$ra_FFmsy_position
      ra_FFmsy_values$si_suffix_current = input$ra_FFmsy_si_suffix

      ra_FFmsy_change$scenarios_changed = FALSE
      ra_FFmsy_change$indices_changed = FALSE
      ra_FFmsy_change$title_x_changed = FALSE
      ra_FFmsy_change$title_y_changed = FALSE
      ra_FFmsy_change$text_size_changed = FALSE
      ra_FFmsy_change$x_min_changed = FALSE
      ...
      filtered_ra_FFmsy(
        list(
          data = ra_df$data %>%
            filter(
              Scenario %in% input$ra_FFmsy_scenarios
            ),
      ...
      title_x_ra_FFmsy(input$ra_FFmsy_title_x)
      title_y_ra_FFmsy(input$ra_FFmsy_title_y)
      text_size_ra_FFmsy(input$ra_FFmsy_text_size)
      x_lim_min_ra_FFmsy(x_min)
      x_lim_max_ra_FFmsy(x_max)
      y_lim_min_ra_FFmsy(y_min)
      y_lim_max_ra_FFmsy(y_max)
      position_ra_FFmsy(input$ra_FFmsy_position)
      si_suffix_ra_FFmsy(input$ra_FFmsy_si_suffix)
    }
  }, ignoreInit = TRUE)
  ...
  output$retrospective_analysis_FFmsy <- renderPlotly({
    ...
    rho_data <- filtered_ra_FFmsy()$rho_data
    ...
    plots <- map(scenarios, function(s) {
      ...
      plot_ly(colors = c("black", ss3col(8))) %>%
        ...
        .plotly_config("retrospective_analysis_FFmsy_plot")
    })
      ...
      layout(
        annotations = list(
          list(
            ...
            xref = "paper",
            yref = "paper",
            text = .format_title(title_x_ra_FFmsy()),
            showarrow = FALSE,
            font = list(
              size = 20
            )
