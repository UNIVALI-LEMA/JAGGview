...
.traj_Catch_server <- function(
  ...
  observeEvent(input$traj_Catch_scenarios, {
    if (!setequal(input$traj_Catch_scenarios, 
      ...
      traj_Catch_change$scenarios_changed = FALSE
    }
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$traj_Catch_title_x, {
    if(!identical(input$traj_Catch_title_x, traj_Catch_values$title_x_current)){
      ...
      traj_Catch_change$title_x_changed = FALSE
    }
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$traj_Catch_title_y, {
    if(!identical(input$traj_Catch_title_y, traj_Catch_values$title_y_current)){
      ...
      traj_Catch_change$title_y_changed = FALSE
    }
  }, ignoreInit = TRUE)

  observeEvent(input$traj_Catch_color, {
    if (!identical(input$traj_Catch_color, traj_Catch_values$color_current)) {
      traj_Catch_change$color_changed = TRUE
    }
    else {
      traj_Catch_change$color_changed = FALSE
    }
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$traj_Catch_x_min, {
    if (!identical(input$traj_Catch_x_min, traj_Catch_values$x_min_current)) {
      ...
      traj_Catch_change$x_min_changed = FALSE
    }
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$traj_Catch_x_max, {
    if (!identical(input$traj_Catch_x_max, traj_Catch_values$x_max_current)) {
      ...
      traj_Catch_change$x_max_changed = FALSE
    }
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$traj_Catch_y_min, {
    if (!identical(input$traj_Catch_y_min, traj_Catch_values$y_min_current)) {
      ...
      traj_Catch_change$y_min_changed = FALSE
    }
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$traj_Catch_y_max, {
    if (!identical(input$traj_Catch_y_max, traj_Catch_values$y_max_current)) {
      ...
      traj_Catch_change$y_max_changed = FALSE
    }
  }, ignoreInit = TRUE)

  observeEvent(input$traj_Catch_si_suffix, {
    if (
      !identical(
        input$traj_Catch_si_suffix, traj_Catch_values$si_suffix_current
      )
    ) {
      traj_Catch_change$si_suffix_changed = TRUE
    }
    else {
      traj_Catch_change$si_suffix_changed = FALSE
    }
  }, ignoreInit = TRUE)

  status_sliders_traj_Catch <- reactive({
    req(input$navmenu == "tab_trajectories" && 
      input$trajectories_tabs == "tab_traj_Catch")
    vec <- unlist(reactiveValuesToList(traj_Catch_change))
    
    enable <- any(vec) && !.is_empty(input$traj_Catch_scenarios)
  ...
  observeEvent(input$confirm_button, {
    ...
    if (
      ...
      y_min <- input$traj_Catch_y_min
      y_max <- input$traj_Catch_y_max
      ...
      if (!is.na(y_min) && !is.na(y_max) && y_min > y_max) {
        ...
        y_max <- tmp_y
        updateSelectInput(
          session, inputId = "traj_Catch_y_min", selected = y_min
        )
        updateSelectInput(
          session, inputId = "traj_Catch_y_max", selected = y_max
        )
        showNotification(
      ...
      if (!is.na(x_min) && !is.na(x_max) && x_min > x_max) {
        ...
        x_max <- tmp_x
        updateSelectInput(
          session, inputId = "traj_Catch_x_min", selected = x_min
        )
        updateSelectInput(
          session, inputId = "traj_Catch_x_max", selected = x_max
        )
        showNotification(
      ...
      traj_Catch_values$scenarios_current = input$traj_Catch_scenarios
      traj_Catch_values$title_x_current = input$traj_Catch_title_x
      traj_Catch_values$title_y_current = input$traj_Catch_title_y
      traj_Catch_values$color_current = input$traj_Catch_color
      traj_Catch_values$x_min_current = x_min
      traj_Catch_values$x_max_current = x_max
      traj_Catch_values$y_min_current = y_min
      traj_Catch_values$y_max_current = y_max
      traj_Catch_values$si_suffix_current = input$traj_Catch_si_suffix
      ...
      title_y_traj_Catch(input$traj_Catch_title_y)
      palette_traj_Catch(input$traj_Catch_color)
      x_lim_min_traj_Catch(x_min)
      x_lim_max_traj_Catch(x_max)
      y_lim_min_traj_Catch(y_min)
      y_lim_max_traj_Catch(y_max)
      si_suffix_traj_Catch(input$traj_Catch_si_suffix)
    }
  }, ignoreInit = TRUE)
