...
.retrospective_analysis_MSY_server <- function(
  ...
  observeEvent(input$confirm_button, {
    if (
      ...
      ra_MSY_values$x_min_current = x_min
      ra_MSY_values$x_max_current = x_max
      ra_MSY_values$y_min_current = y_min
      ra_MSY_values$y_max_current = y_max
      ra_MSY_values$position_current = input$ra_MSY_position
      ra_MSY_values$si_suffix_current = input$ra_MSY_si_suffix
      ...
      x_lim_min_ra_MSY(x_min)
      x_lim_max_ra_MSY(x_max)
      y_lim_min_ra_MSY(y_min)
      y_lim_max_ra_MSY(y_max)
      position_ra_MSY(input$ra_MSY_position)
      si_suffix_ra_MSY(input$ra_MSY_si_suffix)
    }
  }, ignoreInit = TRUE)
  ...
  output$retrospective_analysis_MSY <- renderPlotly({
    ...
    plots <- map(scenarios, function(s) {
      ...
      plot_ly(colors = c("black", ss3col(8))) %>%
        ...
        .plotly_config("retrospective_analysis_MSY_plot")
    })
